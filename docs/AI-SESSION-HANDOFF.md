...
- **A confirmed push isn't a landed push — verify with a fresh pull, not the person's word alone.** 2026-08-16 session: the person said "I've pushed the diff," and an immediate fresh pull appeared to confirm the `tools/lib/validation.js` author-field fix was present. A later fresh pull (same session, ~10 messages on) showed it missing — 4 of 5 files from the prior session's diff had landed, that one hadn't. Root cause unclear (partial apply, a missed hunk, something else) and doesn't matter — what matters is the check itself caught a real gap that would otherwise have shipped a second silent regression. Re-verify with a fresh pull before trusting "already fixed," even when an earlier check in the same session said otherwise.
...
37 books, 23 memes, 13 people, 5 context essays, 0 resources (entity type exists, no content yet — see below). `node tools/validate.js` and `node tools/sync.js` both run clean (only informational fuzzy-duplicate warnings on the three Ni Kuang titles, which are genuinely separate books by the same author — expected, not an error). Full breakdown with rationale for every non-obvious connection: `docs/seed-list.md`.

**Scope decision made 2026-08-16:** this project stays books-only. Cross-media expansion (film/Cantopop/internet slang), comics/periodicals, and a public crowdsourced verification network — all raised as options from an external (Gemini) brainstorm — were explicitly declined; other Hong Kong culture content belongs in a separate knowledge base, not this one.

**Schema evolution 2026-08-16 (see `schemas/book.schema.json`, `.github/instructions/books.instructions.md`, `.github/instructions/resources.instructions.md` for the authoritative current shape):**
- `memes:` changed from a bare-string array to `{slug, relation}[]`, `relation` one of `exemplifies`/`originates`/`subverts`/`critiques`. All 35 books that had `memes:` were migrated via the one-time `tools/migrate-typed-edges.js` script, all defaulted to `relation: exemplifies` (the only meaning the field carried before). Confirmed via a full diff that only the `memes:` field changed in every migrated file, nothing else.
- New optional `related_books:` field — book-to-book relations (`fictionalizes`/`responds_to`/`inherits`/`criticizes`), distinct from `memes:`'s book-to-motif relations. Not yet populated on any book.
- New optional `availability:` field — ecopy status, and required `physical_access` resources when no ecopy exists.
- New 5th entity type, `knowledge/resources/`, for physical-copy access points (library, lending app, bookstore) — same "plain-name placeholder until 2+ books cite it, then promote to a real page" pattern as `knowledge/people/`. Zero resource pages exist yet since zero books have populated `availability:` yet.
- `graph.json` edges now carry `relation` (for `book-meme` and `book-book` kinds) and a new `book-resource` edge kind exists.
...
- ~~`schemas/book.schema.json`, `schemas/meme.schema.json`, `schemas/context.schema.json` don't exist yet~~ — **done.** All schemas exist (now five, with `people.schema.json` and `resource.schema.json`) and `tools/validate.js` enforces schema conformance, no-isolated-books, meme/related_books/physical_access reference resolution, and duplicate/fuzzy-duplicate detection.
- ~~The contribution-prompt-builder tool depends on `knowledge/.generated/entity-index.json`~~ — **confirmed wired up**, now includes `resource` entries too.
- **Not automated, and probably can't be from frontmatter alone:** the original-argument disclosure requirement and the "meme header format" convention (see Quality Discipline above). Worth deciding whether a lightweight heuristic check is worth building, versus continuing to rely on session-start review.
...
- ~~`individual-vs-system.md`/`shadow-of-globalization.md` discuss 13.67 and 譚劍's work without a real graph edge~~ — still true, not resolved, just no longer new: unchanged since last session, still needs a human call on whether to edit 13.67's `memes:` to add the connection.
- **4 books flagged by `migrate-typed-edges.js`'s heuristic scan, need a human relation-type decision:** `love-in-a-fallen-city`, `portrait-of-fierce-women`, and `the-fall-of-hong-kong` against `first-generation` (currently `exemplifies`, script flagged possible `originates`); `millennium-blackout` against `individual-vs-system` (currently `exemplifies`, script flagged possible `subverts`/`critiques`). The script deliberately didn't auto-assign these — same "verify, don't pattern-match" reasoning as the fabricated-title lesson above.
- `related_books:` and `availability:`/`knowledge/resources/` are built and validated but have **zero real content** — next natural task is populating them for a handful of books (the nonfiction/fiction pairs already sharing memes, e.g. `sam-bat-gun`-linked books, are the obvious first `related_books:` candidates; any book known to be out of print is an `availability:` candidate).
